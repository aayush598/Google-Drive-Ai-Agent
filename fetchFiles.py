...
# Create or update the files table to include a category column
cursor.execute('''
        ...
        for file_item in file_list:
            ...
            if file_id not in existing_files:
                cursor.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (file_id, file_name, file_item['mimeType'], file_item['parents'][0]['id'] if 'parents' in file_item else 'No Parent', file_item['createdDate'], file_item['modifiedDate'], is_folder, None))
                conn.commit()
